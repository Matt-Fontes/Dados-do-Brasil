...
const tokenMapbox = 'pk.eyJ1IjoiYXJpbmRhbTE5OTMiLCJhIjoiY2p0bnU2dmtoMHp4ZTN5cGxmZXJpa3BpdiJ9._9GLi1K1ERIMzwpzWSL-PA';
        ...
        imagesToLoad.forEach(img => {
            if (!mapRef.current.hasImage(img)) {
                mapRef.current.loadImage(img, (error, image) => {
                    ...
                    if (!mapRef.current.hasImage(img)) mapRef.current.addImage(img, image);
                });
            }
        });
    };
